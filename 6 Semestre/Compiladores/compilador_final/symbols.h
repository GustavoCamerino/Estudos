...
// Guarda valores de variaveis do tipo float (elas necessitam de uma insercao diferente)
struct FloatConstant_struct {
    char label[32];   // Nome do label
    char value[32];   // Valor
};
typedef struct FloatConstant_struct FloatConstant;


// Estrutura da tabela de simbolos para funcoes
struct st_sym_func {
    int type; // Tipo foi alterado de 'char' para 'int', pois os tipos sao definidos como constantes inteiras
    char name[MAX_TOKSZ];
    type_symbol_table_entry params[MAX_PARAMS]; // eh a struct usada para variaveis
    char label[MAX_CHAR];
    int nparams;
};
...
// Parametro "type" foi alteado de 'char' para 'int', pois os tipos sao definidos como constantes inteiras
