...
/**
 * @brief Atribuição para variável inteira
 *
 * @param lexeme_of_id
 * @param type
 */
void gen_assign(char *lexeme_of_id, int type)
{
    if (type == INT)
    {
        fprintf(output_file, ";Atribuicao para variavel inteira\n");
        fprintf(output_file, "pop rax\n");                     // Result of E() is on stack
        fprintf(output_file, "mov [%s], eax\n", lexeme_of_id); // Store in variable
    }
    else
    {
        fprintf(output_file, "[ERRO] Tipo nao suportado para atribuicao: %d\n", type);
    }
}