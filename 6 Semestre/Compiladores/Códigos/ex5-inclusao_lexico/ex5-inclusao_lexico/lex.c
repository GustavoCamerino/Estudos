...
void initLex()
{
    ...
    char c;
    int i = 0; // Índice para armazenar na string

    if (input == NULL)
    {
        printf("Não tem nada no arquivo!\n");
        exit(1);
    }

    while ((c = getc(input)) != EOF) // Corrigido
    {
        if (i < MAX_CHAR - 1) // Evita estouro de buffer
        {
            string[i++] = c;
        }
    }
    string[i] = '\0'; // Finaliza a string corretamente
